...
//	mlockPath.unlock();
	...
	segment_indexs.push_back(pKF->global_index);
	lock.unlock();
//	LOGD("add keyframe end!");

}

void KeyFrameDatabase::downsample(vector<int> &erase_index)
{
//	LOGD("resample keyframe begin!");
	unique_lock<mutex> lock(mMutexkeyFrameList);
...
{
	//LOGD("optimizae pose graph begin!");
	...
	options.max_solver_time_in_seconds = (SOLVER_TIME * 3) * 20; //50;// SOLVER_TIME * 3;
	...
	//LOGD("optimizae pose graph end!");

}

void KeyFrameDatabase::updateVisualization()
{
	//LOGD("updateVisualization begin");
	unique_lock<mutex> mlockPath(mPath);
...
//		pose_stamped.pose = odometry.pose.pose;
		...
		refine_path.push_back(P.cast<float>());
		segment_indexs.push_back((*it)->global_index);

	}
	//LOGD("updateVisualization end");
}
