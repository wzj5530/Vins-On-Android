...
void printStatistics(const Estimator &estimator, double t)
{
    ...
    LOGD_STREAM("orientation: " << estimator.Vs[WINDOW_SIZE].transpose());
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        LOGD("calibration result for camera %d", i);
        LOGD_STREAM("extirnsic tic: " << estimator.tic[i].transpose());
        LOGD_STREAM("extrinsic ric: " << Utility::R2ypr(estimator.ric[i]).transpose());
    ...
    sum_of_time += t;
    sum_of_calculation++;
    LOGD("vo solver costs: %f ms", t);
    LOGD("average of time %f ms", sum_of_time / sum_of_calculation);

    sum_of_path += (estimator.Ps[WINDOW_SIZE] - last_path).norm();
    last_path = estimator.Ps[WINDOW_SIZE];
    LOGD("sum of path %f", sum_of_path);
}
...
*/
        ...
        LOGD("xxxx foutC = %s, foutC.close() pub", VINS_RESULT_PATH.c_str());
